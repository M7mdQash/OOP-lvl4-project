...
//down here we check if the rook/castle hasnt moved so it can make a castle move
        if (this.isFirstMove != true || rook.isFirstMove != true) {
            System.out.println("Cannot castle if king or rook has already moved");
            return -1;
        }
        //if conditions dont apply then we procead to the check if check conditions
        if (Board.isPathClear(this.getX(), this.getY(), rook.getX(), rook.getY()) != true) {
            System.out.println("Cannot castle across a line of check");
            return -1;
        }
        //this now moves to actual castle move

        if (this.getColor() == Color.WHITE) {
            //using our given rules for getting a casltle move we seek the last letter and move the castle accordingly
        ...
        //this is the code for the black side castle move
